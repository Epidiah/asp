...
(defn fetch-entries!
  ...
  the `process-fn` to transform the data with,
  the atom to `store` the transformed data in,
  and a `loc-vec` describing where the data should be stored."
  [url year process-fn store loc-vec]
  (let [cb (fn [_]
             (this-as ^js this-is-a-bad-idea
               (let [status   (.getStatus this-is-a-bad-idea)
                 ...
                 (when (and (<= 200 status) (> 400 status))
                   (swap! store update-in loc-vec
                          merge (process-fn year response))))))]
...
(defn initialize-entries!
  ...
  `process-fn` should be a function that transforms the raw data into a
  usable format. `store` should be an atom in which you will store
  the data as it is received."
  [process-fn store loc-vec]
  (doseq [yr archive-search-range]
    (fetch-entries! (str "csv/" yr ".csv") yr process-fn store loc-vec)))
...
(def update-url-vectors
  ...
  (update-suffixed
   ...
   (fn [s]
     (if (str/blank? s)
       []
       (let [chunks      (partition-by url-str? (str/split s #"\s+"))
             text-chunk? (fn [f] ((complement url-str?) f))
